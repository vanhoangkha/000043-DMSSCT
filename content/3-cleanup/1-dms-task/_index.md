+++
title = "Database Migration Task"
date = 2020
weight = 1
chapter = false
pre = "<b>3.1. </b>"
+++

**Contents**
- [Delete the Database Migration Task](#delete-the-database-migration-task)

---

#### Delete the Database Migration Task

1. Access [**Database Migration Service (AWS DMS) console**](https://console.aws.amazon.com/dms/).
2. On the left-hand menu click on **Database migration tasks**, and select the migration tasks that you created one at a time.

![Delete](/images/3/1.png?width=90pc)

3. Click on the **Actions** button on the right-hand side, and then select **Stop**.

![Delete](/images/3/2.png?width=90pc)

4. Confirm that you want to stop the migration task.

![Delete](/images/3/3.png?width=90pc)

5. After the status of the migration tasks changes to **Stopped**, click on the **Actions** button again, and then select **Delete**.

![Delete](/images/3/4.png?width=90pc)

6. Confirm that you want to delete the migration task.

![Delete](/images/3/5.png?width=90pc)

Continue to [Delete the DMS Endpoints](../2-dms-endpoints/)...
